...
def trace_comp(comp_id, name):
    ...
    for y in range(max(0,ys.min()//10*10), min(ys.max()+10, h), 10):
        row = ''
...
trace_comp(13, 'ring left+bottom')
trace_comp(4, 'ring upper-left')
trace_comp(3, 'ring top + entrance')
trace_comp(2, 'top line + NW/NE corners')
