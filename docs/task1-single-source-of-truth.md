...
> [!IMPORTANT]
> 本文件為《車輛與機器人手臂》專案「任務一全自動巡線與圓環導航」之唯一權威規格書 (Single Source of Truth)。本報告**直接採用使用者最新上傳實拍照片，經 90° 旋轉校正與 OpenCV 透視變換 (`cv2.warpPerspective`) 轉為 100 cm (東西向長) × 70 cm (南北向寬) 之標準長方形正視鳥瞰圖，圖上包含實體比例尺 (Scale Bar，10 px = 1 cm)**，並完整記錄**修正後 100% 首尾無縫連貫之規劃路徑**（每一階段終點即下一階段起點，無斷點）。
>
> **2026-08-19 修訂：任務改為連續繞圈。** 原 Phase 11（返程入庫，回發車區）已刪除。車輛完成 Phase 10 抵達 T 形路口後**直行通過**，接回 Phase 2，無限繞行。Phase 1 僅進場時執行一次。因此**每圈會經過三個路口**：圓環入口（對稱全黑，右轉 90°）、圓環出口（右分岔，右轉 90°）、T 路口（右分岔，**直行通過**）。
>
> **2026-08-20 訂正：`1111` 全黑訊號並非全程唯一。** Start T、圓環入口、圓環出口三個實體位置都會讀到 `1111`，平整直線段也可能因紙張不平或感測器誤讀偶爾跳出 `0111` 之類的雜訊。系統不靠任何單一訊號的「無歧義性」分辨路口身份——判定完全靠三件事疊加：**序列位置**（依固定順序，不問訊號是哪個路口）、**持續時間門檻**（過濾短暫雜訊）、**距離閘門**（過濾離上一個已確認路口太近的重複讀值）。圓環入口在這套機制裡就是序列第 2 個節點，沒有特殊的「同步」地位。詳見 [carbot.ir_route 模組說明](../src/carbot/ir_route.py) 與 [tasks/ir-sensor-tracking/design.md](../tasks/ir-sensor-tracking/design.md)。
>
...
|---|---|---|---|
| **Phase 1** | **發車啟動對齊段**（**僅首圈**） | **自發車區直行駛出**，沿車道軸心 x=702 由 (702,505) 直行至 T 路口 (702,405)。（黑線圖長 10.0 cm；車輪旋轉中心另需約 18 cm 對齊考量）。**僅進場時執行一次，之後每圈都是直行通過 T 路口，不再走這一段** | **10.0 cm** |
| **Phase 2** | **T路口東向段** | **首圈**：自 Phase 1 抵達 T 形路口 (702,405)，90° 順時針右轉往東 (3點鐘)。**之後每圈**：自 Phase 10 由西向東**直行通過**該路口，不轉彎。兩者皆續行 16.0 cm 至 (862,405)，不含 ARC 1 弧長 | **16.0 cm** |
...
| ~~Phase 11~~ | ~~返程入庫段~~ | **已刪除（2026-08-19）**。任務改為連續繞圈，車輛抵達 T 形路口中心後**直行通過往東**，接回 Phase 2 起點，不返回發車區 | — |
| **每圈** | **連續繞圈里程** | **Phase 2 → Phase 10，車輪軸心一圈行駛距離** | **≈244 cm** |
