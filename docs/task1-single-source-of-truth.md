...
>
> **2026-08-19 修訂：任務改為連續繞圈。** 原 Phase 11（返程入庫，回發車區）已刪除。車輛完成 Phase 10 抵達 T 形路口後**直行通過**，接回 Phase 2，無限繞行。Phase 1 僅進場時執行一次。因此**每圈會經過三個路口**：圓環入口（對稱全黑，右轉 90°）、圓環出口（右分岔，右轉 90°）、T 路口（右分岔，**直行通過**）。後兩者的感測器訊號完全相同，靠圓環入口那個無歧義的全黑訊號做每圈同步 —— 見 [tasks/ir-sensor-tracking/design.md](../tasks/ir-sensor-tracking/design.md)。
>
